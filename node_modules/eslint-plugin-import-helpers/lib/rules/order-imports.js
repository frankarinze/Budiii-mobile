...
function getRegExpGroups(ranks) {
	return Object.keys(ranks).filter(isRegularExpressionGroup).map(rank => [rank, new RegExp(rank.slice(1, rank.length - 1))]);
}
...
function makeNewlinesBetweenReport(context, imported, newlinesBetweenImports) {
	...
	imported.slice(1).forEach(function (currentImport) {
		...
		if (newlinesBetweenImports === 'always' || newlinesBetweenImports === 'always-and-inside-groups') {
			...
			} else if (currentGroupRank === previousGroupRank && emptyLinesBetween > 0 && newlinesBetweenImports !== 'always-and-inside-groups') {
				context.report({
					node: previousImport.node,
...
function getAlphabetizeConfig(options) {
	...
	if (typeof order !== 'string') {
		throw new Error('Incorrect alphabetize config: `order` property should be ' + 'a string, but `' + JSON.stringify(typeof order) + '` found instead.');
	} else if (['ignore', 'asc', 'desc'].indexOf(order) === -1) {
		throw new Error('Incorrect alphabetize config: `order` property should be ' + 'either `ignore`, `asc` or `desc`, but `' + JSON.stringify(order) + '` found instead.');
	}

	if (typeof ignoreCase !== 'boolean') {
		throw new Error('Incorrect alphabetize config: ignoreCase should be ' + 'a boolean, but `' + JSON.stringify(typeof ignoreCase) + '` found instead.');
	}
